...
namespace _2D_Array_Playground
{
    internal class Program
    {
        ...
        static void WriteArray(int[,] arrayToWrite)
        {
            for (int i = 0; i < arrayToWrite.GetLength(1); i++)
            {
                for (int j = 0; j < arrayToWrite.GetLength(1); j++)
                {
                    Console.Write(arrayToWrite[i, j] + " ");
                    if (arrayToWrite[i, j] < 10) Console.Write(" ");
                }
                Console.WriteLine();
            }
            Console.WriteLine();
